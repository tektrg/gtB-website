---
title: "GPT-4.1 mini pricing (input/output per 1M tokens)"
description: "Current OpenAI pricing for GPT-4.1 mini: input $0.40/1M tokens, output $1.60/1M — plus a quick way to estimate your bill in GPT Breeze."
pubDate: 2026-03-08T00:00:00.000Z
topicId: "model-pricing-openai-gpt-4-1-mini"
tags: ["pricing", "models", "openai", "gpt-4-1-mini"]
faq:
  - q: "How much does GPT-4.1 mini cost?"
    a: "List price is $0.40/1M input tokens and $1.60/1M output tokens (USD)."
...
# GPT-4.1 mini pricing (input/output per 1M tokens)
...
This page is the **current price sheet** for **GPT-4.1 mini** (provider: **OpenAI**) and a practical way to estimate costs in GPT Breeze using your own usage pattern.

## Quick pricing (USD per 1M tokens)

| Item | Price | Notes |
|---|---:|---|
| Input tokens | $0.40 / 1M | Prompt + system + context |
| Output tokens | $1.60 / 1M | Model responses |
| Cache read | $0.10 / 1M | If provider supports cached inputs |
...
- **100k input + 20k output** ≈ $0.072 (input $0.04 + output $0.032)
- **1M input + 200k output** ≈ $0.72 (input $0.40 + output $0.32)
...
Pricing data is pulled from GPT Breeze’s internal model catalog (`src/data/models-api.json`) on **2026-03-08**. Providers change prices; if this page looks stale, check the calculator and the provider’s official docs.
