---
title: "Claude Haiku 3.5 (latest) pricing (input/output per 1M tokens)"
description: "Current Anthropic pricing for Claude Haiku 3.5 (latest): input $0.80/1M tokens, output $4/1M — plus a quick way to estimate your bill in GPT Breeze."
pubDate: 2026-03-11T00:00:00.000Z
topicId: "model-pricing-anthropic-claude-3-5-haiku-latest"
tags: ["pricing", "models", "anthropic", "claude-3-5-haiku-latest"]
faq:
  - q: "How much does Claude Haiku 3.5 (latest) cost?"
    a: "List price is $0.80/1M input tokens and $4/1M output tokens (USD)."
...
# Claude Haiku 3.5 (latest) pricing (input/output per 1M tokens)
...
This page is the **current price sheet** for **Claude Haiku 3.5 (latest)** (provider: **Anthropic**) and a practical way to estimate costs in GPT Breeze using your own usage pattern.

## Quick pricing (USD per 1M tokens)

| Item | Price | Notes |
|---|---:|---|
| Input tokens | $0.80 / 1M | Prompt + system + context |
| Output tokens | $4 / 1M | Model responses |
| Cache read | $0.08 / 1M | If provider supports cached inputs |
| Cache write | $1 / 1M | If provider charges to write cache |
...
- **100k input + 20k output** ≈ $0.16 (input $0.08 + output $0.08)
- **1M input + 200k output** ≈ $1.60 (input $0.80 + output $0.80)
...
Pricing data is pulled from GPT Breeze’s internal model catalog (`src/data/models-api.json`) on **2026-03-11**. Providers change prices; if this page looks stale, check the calculator and the provider’s official docs.
