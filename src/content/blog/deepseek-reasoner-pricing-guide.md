---
title: "DeepSeek Reasoner pricing (input/output per 1M tokens)"
description: "Current DeepSeek pricing for DeepSeek Reasoner: input $0.28/1M tokens, output $0.42/1M — plus a quick way to estimate your bill in GPT Breeze."
pubDate: 2026-03-14T00:00:00.000Z
topicId: "model-pricing-deepseek-deepseek-reasoner"
tags: ["pricing", "models", "deepseek", "deepseek-reasoner"]
faq:
  - q: "How much does DeepSeek Reasoner cost?"
    a: "List price is $0.28/1M input tokens and $0.42/1M output tokens (USD)."
...
# DeepSeek Reasoner pricing (input/output per 1M tokens)
...
This page is the **current price sheet** for **DeepSeek Reasoner** (provider: **DeepSeek**) and a practical way to estimate costs in GPT Breeze using your own usage pattern.

## Quick pricing (USD per 1M tokens)

| Item | Price | Notes |
|---|---:|---|
| Input tokens | $0.28 / 1M | Prompt + system + context |
| Output tokens | $0.42 / 1M | Model responses |
| Cache read | $0.028 / 1M | If provider supports cached inputs |
...
- **100k input + 20k output** ≈ $0.036 (input $0.028 + output $0.008)
- **1M input + 200k output** ≈ $0.36 (input $0.28 + output $0.084)
...
Pricing data is pulled from GPT Breeze’s internal model catalog (`src/data/models-api.json`) on **2026-03-13**. Providers change prices; if this page looks stale, check the calculator and the provider’s official docs.
