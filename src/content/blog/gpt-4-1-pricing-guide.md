---
title: "GPT-4.1 pricing (input/output per 1M tokens)"
description: "Current OpenAI pricing for GPT-4.1: input $2/1M tokens, output $8/1M — plus a quick way to estimate your bill in GPT Breeze."
pubDate: 2026-03-05T00:00:00.000Z
topicId: "model-pricing-openai-gpt-4-1"
tags: ["pricing", "models", "openai", "gpt-4-1"]
faq:
  - q: "How much does GPT-4.1 cost?"
    a: "List price is $2/1M input tokens and $8/1M output tokens (USD)."
...
# GPT-4.1 pricing (input/output per 1M tokens)
...
This page is the **current price sheet** for **GPT-4.1** (provider: **OpenAI**) and a practical way to estimate costs in GPT Breeze using your own usage pattern.

## Quick pricing (USD per 1M tokens)

| Item | Price | Notes |
|---|---:|---|
| Input tokens | $2 / 1M | Prompt + system + context |
| Output tokens | $8 / 1M | Model responses |
| Cache read | $0.50 / 1M | If provider supports cached inputs |
...
- **100k input + 20k output** ≈ $0.36 (input $0.20 + output $0.16)
- **1M input + 200k output** ≈ $3.60 (input $2 + output $1.60)
...
Pricing data is pulled from GPT Breeze’s internal model catalog (`src/data/models-api.json`) on **2026-03-05**. Providers change prices; if this page looks stale, check the calculator and the provider’s official docs.
