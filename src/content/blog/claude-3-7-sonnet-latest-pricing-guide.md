---
title: "Claude Sonnet 3.7 (latest) pricing (input/output per 1M tokens)"
description: "Current Anthropic pricing for Claude Sonnet 3.7 (latest): input $3/1M tokens, output $15/1M — plus a quick way to estimate your bill in GPT Breeze."
pubDate: 2026-03-10T00:00:00.000Z
topicId: "model-pricing-anthropic-claude-3-7-sonnet-latest"
tags: ["pricing", "models", "anthropic", "claude-3-7-sonnet-latest"]
faq:
  - q: "How much does Claude Sonnet 3.7 (latest) cost?"
    a: "List price is $3/1M input tokens and $15/1M output tokens (USD)."
...
# Claude Sonnet 3.7 (latest) pricing (input/output per 1M tokens)
...
This page is the **current price sheet** for **Claude Sonnet 3.7 (latest)** (provider: **Anthropic**) and a practical way to estimate costs in GPT Breeze using your own usage pattern.

## Quick pricing (USD per 1M tokens)

| Item | Price | Notes |
|---|---:|---|
| Input tokens | $3 / 1M | Prompt + system + context |
| Output tokens | $15 / 1M | Model responses |
| Cache read | $0.30 / 1M | If provider supports cached inputs |
| Cache write | $3.75 / 1M | If provider charges to write cache |
...
- **100k input + 20k output** ≈ $0.60 (input $0.30 + output $0.30)
- **1M input + 200k output** ≈ $6 (input $3 + output $3)
...
Pricing data is pulled from GPT Breeze’s internal model catalog (`src/data/models-api.json`) on **2026-03-10**. Providers change prices; if this page looks stale, check the calculator and the provider’s official docs.
